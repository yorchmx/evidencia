    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           ...
           if(Integer.parseInt(Peso) <= 0) {
               ...
               if(ck!=null){
                   for (Cookie ckl : ck) {
                       if (ckl.getName().equals("usuario")) {
                           user = ckl.getValue();
                       }
                   }
               }
               //3 Consultas  base de datos.
               Consultas con = new Consultas();
               Consultas con2 = new Consultas();
               Consultas con3 = new Consultas();
               
               IMC imc = new IMC(Peso, con3.alturaUser(user));
               imc.calculoIMC();
               con.registroP(user, imc.getPeso());
               con2.registroC(user, imc.getIMC());
